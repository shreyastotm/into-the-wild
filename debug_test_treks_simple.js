  ...
  try {
    ...
    if (testTreks?.length > 0) {
      testTreks.forEach(trek => {
        console.log(`  - ${trek.name} (ID: ${trek.trek_id}, Status: ${trek.status})`);
      });
      ...
      if (regError) {
        ...
        if (registrations?.length > 0) {
          registrations.forEach(reg => {
            console.log(`  - Trek ID ${reg.trek_id}: User ID ${reg.user_id} - ${reg.payment_status}`);
          });
          ...
          if (usersError) {
            ...
            if (users?.length > 0) {
              users.forEach(user => {
                console.log(`  - User ID ${user.user_id}: ${user.email} (${user.full_name})`);
              });
            }
          }
        }
