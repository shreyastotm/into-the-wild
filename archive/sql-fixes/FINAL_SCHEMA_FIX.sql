-- FINAL SCHEMA FIX for trek event creation errors
...
ADD COLUMN IF NOT EXISTS event_type VARCHAR(50) DEFAULT 'trek';

-- Copy data from existing columns if they exist
UPDATE public.trek_events 
SET name = trek_name 
WHERE name IS NULL AND trek_name IS NOT NULL;

UPDATE public.trek_events 
SET base_price = cost 
WHERE base_price IS NULL AND cost IS NOT NULL;
...
-- ==============================================
-- ISSUE 3: Fix the cost_type mapping in the application
-- ==============================================
-- The application is using different cost types than what the database allows
-- We need to either:
-- 1. Update the database constraint to allow more cost types, OR
-- 2. Update the application to use the correct cost types

-- Option 1: Update the constraint to allow more cost types
ALTER TABLE public.trek_costs 
DROP CONSTRAINT IF EXISTS trek_costs_cost_type_check;

ALTER TABLE public.trek_costs 
ADD CONSTRAINT trek_costs_cost_type_check 
CHECK (cost_type IN (
    'ACCOMMODATION', 'TICKETS', 'LOCAL_VEHICLE', 'GUIDE', 'OTHER',
    'transportation', 'accommodation', 'food', 'permits', 'guide', 
    'equipment', 'insurance', 'other'
));
