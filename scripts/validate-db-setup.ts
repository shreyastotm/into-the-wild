...
class DatabaseSetupValidator {
  ...
  private checkMigrations(): void {
    ...
    if (exists) {
      try {
        ...
        const hasConsolidated = files.includes('20260101000000_comprehensive_schema_consolidation.sql');
        this.results.push({
          check: 'Consolidated migration',
          status: hasConsolidated ? '✅' : '❌',
          details: hasConsolidated ? 'Migration file found' : 'Migration file missing'
        });
      } catch (error) {
        this.results.push({
  ...
  private printResults(): void {
    ...
    console.log(`📊 Summary: ${success} passed, ${warnings} warnings, ${errors} errors\n`);
