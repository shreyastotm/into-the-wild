...
try {
  ...
  const schemaDump = execSync('npx supabase db dump --schema-only --data-only=false', { 
    encoding: 'utf8',
    cwd: process.cwd()
  });
  ...
  const tableDump = execSync('npx supabase db dump --schema-only --data-only=false --table="public.*"', { 
    encoding: 'utf8',
    cwd: process.cwd()
  });
  ...
  const typesDump = execSync('npx supabase db dump --schema-only --data-only=false --table="public.*" | grep -E "(CREATE TYPE|DROP TYPE|ALTER TYPE)"', { 
    encoding: 'utf8',
    cwd: process.cwd(),
    shell: true
  });
  ...
  const functionsDump = execSync('npx supabase db dump --schema-only --data-only=false | grep -A 50 -E "(CREATE FUNCTION|CREATE OR REPLACE FUNCTION)"', { 
    encoding: 'utf8',
    cwd: process.cwd(),
    shell: true
  });
  ...
  const policiesDump = execSync('npx supabase db dump --schema-only --data-only=false | grep -A 10 -E "(CREATE POLICY|DROP POLICY|ALTER TABLE.*ENABLE ROW LEVEL SECURITY)"', { 
    encoding: 'utf8',
    cwd: process.cwd(),
    shell: true
  });
