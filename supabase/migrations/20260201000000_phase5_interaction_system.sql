...
-- Tables for profile completion tracking
CREATE TABLE IF NOT EXISTS public.profile_completion_stages (
...
CREATE INDEX IF NOT EXISTS idx_profile_completion_user_id ON public.profile_completion_stages(user_id);
CREATE INDEX IF NOT EXISTS idx_profile_completion_stage ON public.profile_completion_stages(stage);
CREATE INDEX IF NOT EXISTS idx_profile_completion_status ON public.profile_completion_stages(status);
CREATE INDEX IF NOT EXISTS idx_profile_completion_percentage ON public.profile_completion_stages(completion_percentage);
...
ALTER TABLE public.profile_completion_stages ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can manage their own profile completion" ON public.profile_completion_stages
  FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Admins can view all profile completion" ON public.profile_completion_stages
...
CREATE POLICY "Users can view tags on accessible images" ON public.image_tags
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.image_tag_assignments ita
      JOIN public.user_trek_images uti ON uti.image_id = ita.image_id
      JOIN public.trek_registrations tr ON tr.trek_id = uti.trek_id
      WHERE ita.tag_id = image_tags.tag_id AND (
...
CREATE POLICY "Admins can manage all tags" ON public.image_tags
...
GRANT SELECT, INSERT, UPDATE ON public.profile_completion_stages TO authenticated;
...
BEGIN
  ...
  INSERT INTO public.profile_completion_stages (
    user_id,
    stage,
    status,
