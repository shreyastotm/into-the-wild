...
import { AdminTrekEvent, FormSubmissionData } from '@/components/trek/create/types';
...
export default function CreateTrekEvent() {
  ...
  const handleFormSubmit = async (data: FormSubmissionData) => {
    try {
      ...
      if (!trekData.name || !trekData.start_datetime || !trekData.base_price || !trekData.max_participants) {
      ...
      const sanitizedTrekData = {
        ...
        end_datetime: trekData.end_datetime ? new Date(trekData.end_datetime).toISOString() : null,
      ...
      if (tentData && tentData.length > 0 && trekData.event_type === 'camping') {
        ...
        const { error: tentError } = await supabase
          ...
          .insert(tentAssignments);

        if (tentError) {
          throw new Error(`Failed to assign tent inventory: ${tentError.message}`);
        }
      }

      toast({
        title: 'Event Created Successfully',
        description: 'Your trek event has been created and is now in draft status.',
        variant: 'default',
      });
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to create trek event';
      toast({
