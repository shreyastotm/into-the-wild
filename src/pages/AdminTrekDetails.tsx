  ...
  const fetchTrekDetails = async (id: number) => {
    try {
      ...
      const normalizedData: TrekEvent = {
        ...
        trek_name: trekData.trek_name || trekData.name || `Trek ${trekData.trek_id}`,
  ...
  const fetchParticipants = async (id: number) => {
    try {
      ...
      const combinedParticipants = registrationsData.map(reg => {
        ...
        const userDetail = userId ? usersData.find(u => u?.user_id === userId) : null;
            ...
            <CardHeader className="bg-primary/5">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardDescription>
                    Trek ID: {trek.trek_id} | {trek.transport_mode ? `Transport: ${trek.transport_mode}` : 'No transport'} | 
                    Participants: {participants.length}/{trek.max_participants}
                  </CardDescription>
                </div>
            ...
            <CardContent className="mt-4">
              ...
              {trek.description && (
                <div className="border p-4 rounded-md mb-4">
                  ...
                  <p className="text-sm whitespace-pre-wrap">{trek.description}</p>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <TabsContent value="participants">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {participants.length} of {trek.max_participants} slots filled
                  </CardDescription>
                </CardHeader>
                <CardContent>
