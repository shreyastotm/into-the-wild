...
export default function CreatePastEvent() {
  ...
  async function uploadImageAndGetUrl(file: File, trekId: number, position: number): Promise<string> {
    ...
    const { data: publicUrlData } = supabase.storage.from('trek-images').getPublicUrl(filePath);
    ...
    try {
      ...
      images.forEach((file, index) => {
        ...
        uploads.push((async () => {
          ...
          const { error: imgErr } = await supabase.from('trek_event_images').insert({
            trek_id: trekId,
            image_url: url,
            position: index + 1,
          });
      ...
      toast({ title: 'Past Event Created', description: 'Your past trek has been saved.' });
      navigate('/admin/events');
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : 'Failed to create past event';
      toast({ title: 'Error', description: msg, variant: 'destructive' });
    } finally {
      setSubmitting(false);
    }
  }, [canSubmit, name, description, location, difficulty, date, distanceKm, images, navigate]);
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Rajmachi Night Trek" />
            </div>
            <div>
              <Label htmlFor="location">Location</Label>
              <Input id="location" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g., Lonavala, Maharashtra" />
            </div>
            <div>
            ...
            <div>
              ...
              <Input id="distance" type="number" min="0" step="0.1" value={distanceKm}
                onChange={(e) => setDistanceKm(e.target.value)} placeholder="e.g., 12" />
            </div>
            <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
              {[0,1,2].map(i => (
                <div key={i}>
                  <Label>Image {i+1}</Label>
                  <Input type="file" accept="image/*" onChange={(e) => onSelectImage(i, e.target.files?.[0] ?? null)} />
                </div>
              ))}
            </div>
          </div>
