...
export const EventStatusBadge: React.FC<Omit<TrekStatusBadgeProps, 'status'> & {
  status: 'DRAFT' | 'PUBLISHED' | 'OPEN_FOR_REGISTRATION' | 'REGISTRATION_CLOSED' | 'ONGOING' | 'COMPLETED' | 'CANCELLED';
}> = (props) => <TrekStatusBadge {...props} />;

export const RegistrationStatusBadge: React.FC<Omit<TrekStatusBadgeProps, 'status'> & {
  status: 'REGISTERED' | 'CONFIRMED' | 'WAITLISTED' | 'CANCELLED_REGISTRATION';
}> = (props) => <TrekStatusBadge {...props} />;

export const PaymentStatusBadge: React.FC<Omit<TrekStatusBadgeProps, 'status'> & {
  status: 'PAID' | 'PENDING_PAYMENT' | 'PROOF_UPLOADED' | 'PAYMENT_FAILED';
}> = (props) => <TrekStatusBadge {...props} />;

export const AvailabilityStatusBadge: React.FC<Omit<TrekStatusBadgeProps, 'status'> & {
  status: 'AVAILABLE' | 'LIMITED_SPOTS' | 'FULLY_BOOKED' | 'WAITLIST_ONLY';
}> = (props) => <TrekStatusBadge {...props} />;

export const TransportStatusBadge: React.FC<Omit<TrekStatusBadgeProps, 'status'> & {
  status: 'TRANSPORT_ARRANGED' | 'CARPOOL_AVAILABLE' | 'BUS_TRANSPORT' | 'SELF_TRANSPORT';
}> = (props) => <TrekStatusBadge {...props} />;

// Status badge with count
export const TrekStatusBadgeWithCount: React.FC<TrekStatusBadgeProps & {
  count?: number;
}> = ({ count, ...props }) => (
