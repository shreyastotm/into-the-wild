    ...
    <div className="flex flex-wrap gap-2 items-center text-sm">
      ...
      {options.category && (
        <Badge
          variant="outline"
          className={cn(
            "rounded-full px-2 py-1 cursor-pointer hover:bg-muted/80",
          )}
      ...
      {options.priceRange && (
        ...
        >
          Price:{" "}
          {priceRanges.find((r) => r.value === options.priceRange)?.label ||
            options.priceRange}{" "}
          ×
        </Badge>
      )}
      {options.timeFrame && (
        ...
        >
          Time:{" "}
          {timeFrames.find((t) => t.value === options.timeFrame)?.label ||
            options.timeFrame}{" "}
          ×
        </Badge>
      )}
