...
export const CampingDetailsStep: React.FC<CampingDetailsStepProps> = ({
  ...
  const removeActivity = (dayIndex: number, activityIndex: number) => {
    const currentItinerary = formData.itinerary || { days: [] };
    const updatedDays = [...currentItinerary.days];
    const updatedActivities = (updatedDays[dayIndex].activities || []).filter((_, index) => index !== activityIndex);
    updatedDays[dayIndex] = { ...updatedDays[dayIndex], activities: updatedActivities };
  ...
  const updateVolunteerRole = (roleIndex: number, field: string, value: string | number) => {
    const currentRoles = formData.volunteer_roles || { roles: [] };
  ...
  const removeVolunteerRole = (roleIndex: number) => {
    const currentRoles = formData.volunteer_roles || { roles: [] };
    const updatedRoles = currentRoles.roles.filter((_, index) => index !== roleIndex);
    ...
    <div className="space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Configure the detailed itinerary and volunteer roles for your camping event
        </p>
      </div>
              ...
              <Card key={roleIndex} className="border-l-4 border-l-green-500">
                <CardContent className="pt-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <Input
                      value={role.title}
                      onChange={(e) => updateVolunteerRole(roleIndex, 'title', e.target.value)}
                  ...
                  <Textarea
                    ...
                    onChange={(e) => updateVolunteerRole(roleIndex, 'description', e.target.value)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <Input
                      ...
                      onChange={(e) => updateVolunteerRole(roleIndex, 'requirements', e.target.value)}
                    ...
                    <Input
                      ...
                      onChange={(e) => updateVolunteerRole(roleIndex, 'spots_needed', parseInt(e.target.value) || 1)}
