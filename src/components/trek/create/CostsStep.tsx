    ...
    <div className="space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Add any additional fixed costs for this{" "}
          {formData.event_type?.toLowerCase()} (transportation, accommodation,
          permits, etc.)
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewCost({ ...newCost, pay_by_date: e.target.value })
                }
              />
            </div>
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-medium text-muted-foreground">
                        {
                          costTypes.find((t) => t.value === cost.cost_type)
                            ?.label
                        }
                      </span>
                    ...
                    {cost.pay_by_date && (
                      <p className="text-xs text-blue-600">
                        Due: {new Date(cost.pay_by_date).toLocaleDateString()}
                      </p>
                    )}
                  </div>
                  <Button
      ...
      {costs.length === 0 && (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p className="text-sm">
            Add transportation, accommodation, or other fixed costs above
          </p>
        </div>
      )}
    </div>
