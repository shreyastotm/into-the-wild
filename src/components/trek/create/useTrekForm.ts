...
export const useTrekForm = (initialData?: AdminTrekEvent) => {
  ...
  const handleGpxChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
  ...
  const validateStep = useCallback((step: number): FormValidationResult => {
    ...
    switch (step) {
      ...
      case 2: // Basic Details Step
        ...
        if (formData.start_datetime && formData.end_datetime && formData.end_datetime <= formData.start_datetime) {
  ...
  const updateFormData = useCallback((updater: (prev: AdminTrekEvent) => AdminTrekEvent) => {
    setFormData(updater);
