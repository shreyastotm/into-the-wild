...
export const TrekParticipants: React.FC<TrekParticipantsProps> = ({ 
  ...
  return (
    <div className="space-y-4">
      ...
      {participantCount === 0 ? (
         <p className="text-muted-foreground text-sm">No participants have joined yet.</p>
      ) : (
        <div className="flex flex-wrap gap-4">
          <TooltipProvider delayDuration={100}>
            {sortedParticipants.map(participant => (
              <Tooltip key={participant.id}>
                  ...
                  <div className="flex flex-col items-center text-center w-16">
                    <Avatar className={`h-12 w-12 mb-1 ${participant.isEventCreator ? 'ring-2 ring-primary ring-offset-2' : ''} ${participant.id === currentUser ? 'ring-2 ring-blue-500 ring-offset-1' : ''}`}>
                      ...
                      <AvatarFallback className="bg-muted text-muted-foreground font-medium">
                        {(participant.name || 'A').substring(0, 2).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                    <span className="text-xs font-medium line-clamp-1 break-all">{participant.name || 'User'}</span>
                    {participant.isEventCreator && (
                      <span className="text-[10px] font-bold text-primary">Organizer</span>
                    )}
                ...
                <TooltipContent side="bottom">
                  <p className="font-medium">{participant.name || 'Anonymous User'}</p>
                  {participant.joinedAt && (
                     <p className="text-xs text-muted-foreground">Joined: {new Date(participant.joinedAt).toLocaleDateString()}</p>
                  )}
                  {participant.isEventCreator && <p className="text-xs font-bold text-primary">Event Organizer</p>}
                </TooltipContent>
              </Tooltip>
            ))}

            {/* Placeholder spots visually capped */}   
            {remainingSpots > 0 && Array.from({ length: Math.min(remainingSpots, 8) }).map((_, index) => (
              <div key={`empty-${index}`} className="flex flex-col items-center text-center w-16">
                ...
                <span className="text-xs text-muted-foreground mt-1">Open Spot</span>
              </div>
            ))}
            ...
            {remainingSpots > 8 && (
              <div className="flex flex-col items-center text-center w-16">
                 <div className="h-12 w-12 rounded-full bg-gray-100 border border-dashed border-gray-300 flex items-center justify-center">
                  <span className="text-xs font-medium text-gray-500">+{remainingSpots - 8}</span>
                </div>
