...
const ExpenseCard: React.FC<ExpenseCardProps> = ({
  ...
  const myShare = expense.shares.find(
    (share) => share.user_id === currentUserId,
  );
            ...
            <li key={share.id} className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <Avatar className="h-6 w-6">
                  <AvatarFallback>{share.user_name?.charAt(0)}</AvatarFallback>
                </Avatar>
                <span>
                  {share.user_name}
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium">
                  {formatCurrency(share.amount, "INR")}
                </span>
              </div>
            </li>
          ))}
      ...
      <CardFooter className="flex justify-end gap-2">
        ...
        {!isCreator && myShare && myShare.status === "pending" && (
          <>
            ...
            <Button
              size="sm"
              onClick={() => onPayExpense(myShare.id, myShare.amount)}
            >
              Pay Share
            </Button>
          </>
...
const ExpenseList: React.FC<
  { expenses: Expense[] } & Omit<ExpenseCardProps, "expense">
> = ({ expenses, ...props }) => {
  if (!expenses || expenses.length === 0)
    return (
      <div className="text-center py-10">
        <p className="text-muted-foreground">No expenses here.</p>
      </div>
    );
  return (
    <div className="space-y-4">
      {expenses.map((expense) => (
        <ExpenseCard key={expense.id} expense={expense} {...props} />
      ))}
    </div>
  );
};
...
export const ExpenseSplitting: React.FC<ExpenseSplittingProps> = ({
  ...
  const handlePay = async (shareId: number, amount: number) => {
    ...
    if (success)
      toast({
        title: "Success",
        description: `Paid your share of ${formatCurrency(amount, "INR")}.`,
      });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <SummaryCard
            key="myExpenses"
            title="Your expenses"
            value={summary.myExpenses}
          />
          <SummaryCard
            key="myShares"
            title="Your shares"
            value={summary.myShares}
          />
        </div>
          ...
          <TabsContent value="fixed-costs" className="mt-4">
            {fixedCosts.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                <p>No fixed costs for this trek.</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
