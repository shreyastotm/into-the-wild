...
export default function RegistrationAdmin() {
  ...
  async function fetchRegistrations() {
    ...
    if (error) {
      ...
      const [trekResult, userResult] = await Promise.all([
        supabase
          .from("trek_events")
          .select("trek_id, name, start_datetime")
          .in("trek_id", trekIds),
        supabase
          .from("users")
          .select("user_id, name, email")
          .in("user_id", userIds), // Corrected: select user_id, name
      ]);
      ...
      const trekInfoMap = (trekData || []).reduce(
        (acc, trek) => {
          acc[trek.trek_id] = {
        ...
        {} as Record<number, TrekEventInfo>,
      ); // Use TrekEventInfo type
      ...
      const combinedData = (data || []).map(
        (reg: RawRegistration): RegistrationData => ({
          ...reg,
  ...
  async function deleteRegistration(registration_id: number, trek_id?: number) {
    ...
    if (error) {
      ...
      toast({
        ...
        description: "Duplicate registration removed.",
      });
      fetchRegistrations();
    }
  }
        ...
        {group.map((reg) => (
          <div key={reg.registration_id} className="flex items-center gap-4" data-testid="registrationadmin">
            ...
            <Button
              ...
              variant="destructive"
              onClick={() =>
                deleteRegistration(reg.registration_id, reg.trek_id)
              }
            >
              Delete
            </Button>
  ...
  const nonDuplicateRegistrations = registrations.filter(
    (reg) =>
      !duplicateGroups.some((group) =>
        group.some((dup) => dup.registration_id === reg.registration_id),
      ),
  );
  ...
  return (
    <Card className="mb-8">
      <CardHeader>
        ...
        <div className="text-sm text-gray-500" data-testid="registrationadmin">
          Duplicates are highlighted for review and cleanup.
        </div>
      </CardHeader>
      <CardContent>
        {loading ? (
          <div data-testid="registrationadmin">Loading...</div>
        ) : (
          <div className="space-y-6" data-testid="registrationadmin">
            {duplicateGroups.length === 0 ? (
              <div className="text-green-700" data-testid="registrationadmin">
                No duplicate registrations found.
              </div>
            ) : (
              <div className="mb-6" data-testid="registrationadmin">
                <h2 className="text-xl font-semibold mb-3 text-red-600 flex items-center">
                  <AlertTriangle className="mr-2" /> Duplicate Registrations
                  Found
                </h2>
                  ...
                  <tbody>
                    {nonDuplicateRegistrations.map((reg) => (
                      <tr key={reg.registration_id} className="border-b">
                        ...
                        <td className="p-2">
                          {reg.trek_event?.name || reg.trek_id}
                        </td>
                        <td className="p-2">
                          {format(
                            new Date(reg.booking_datetime),
                            "MMM d, yyyy h:mm a",
                          )}
                        </td>
