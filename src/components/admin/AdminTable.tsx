...
import { MoreHorizontal, Edit, Trash2, Eye, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
...
export const UserAdminTable: React.FC<Omit<AdminTableProps<any>, 'columns'> & {
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      render: (value) => {
        const statusConfig = {
          VERIFIED: { label: 'Verified', variant: 'default' as const, icon: <CheckCircle className="h-3 w-3" /> },
          PENDING: { label: 'Pending', variant: 'secondary' as const, icon: <AlertTriangle className="h-3 w-3" /> },
...
export const RegistrationAdminTable: React.FC<Omit<AdminTableProps<any>, 'columns'> & {
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      render: (value, row) => (
        <div>
          <div className="font-medium">{row.trek_event?.name || 'Unknown Event'}</div>
          <div className="text-sm text-muted-foreground">
            {format(new Date(row.trek_event?.start_datetime), 'MMM dd, yyyy')}
          </div>
        </div>
      ),
    },
    {
      ...
      render: (value) => {
        const statusConfig = {
          ...
          ProofUploaded: { label: 'Proof Uploaded', variant: 'outline' as const },
