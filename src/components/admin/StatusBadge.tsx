...
export const UserStatusBadge: React.FC<Omit<StatusBadgeProps, 'status'> & {
  status: 'VERIFIED' | 'PENDING' | 'REJECTED' | 'BANNED';
}> = (props) => <StatusBadge {...props} />;

export const PaymentStatusBadge: React.FC<Omit<StatusBadgeProps, 'status'> & {
  status: 'PAID' | 'PENDING_PAYMENT' | 'PROOF_UPLOADED' | 'CANCELLED';
}> = (props) => <StatusBadge {...props} />;

export const EventStatusBadge: React.FC<Omit<StatusBadgeProps, 'status'> & {
  status: 'DRAFT' | 'PUBLISHED' | 'ONGOING' | 'COMPLETED' | 'CANCELLED';
}> = (props) => <StatusBadge {...props} />;

export const RegistrationStatusBadge: React.FC<Omit<StatusBadgeProps, 'status'> & {
  status: 'REGISTERED' | 'CONFIRMED' | 'WAITLISTED' | 'CANCELLED';
}> = (props) => <StatusBadge {...props} />;

export const FileStatusBadge: React.FC<Omit<StatusBadgeProps, 'status'> & {
  status: 'UPLOADED' | 'PROCESSING' | 'FAILED';
}> = (props) => <StatusBadge {...props} />;

// Status badge with count
export const StatusBadgeWithCount: React.FC<StatusBadgeProps & {
  count?: number;
}> = ({ count, ...props }) => (
...
export const StatusIndicator: React.FC<{
  ...
  const getStatusConfig = (status: string) => {
    const statusMap: Record<string, {
      ...
      'INACTIVE': { icon: <Pause />, color: 'text-gray-500' },
    };
