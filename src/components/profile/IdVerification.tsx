...
const verificationTiers: VerificationTier[] = [
  ...
  },
  {
    ...
    docsRequired: 4,
    benefits: [
      "All trek registrations",
      "Government permit treks",
      "Priority booking",
    ],
    color: "purple",
  },
...
const IdVerification = () => {
  ...
  const [currentTier, setCurrentTier] = useState<"auto" | "quick" | "full">(
    "auto",
  );
  ...
  const getCurrentTier = (): "auto" | "quick" | "full" => {
    if (verification_status === "VERIFIED") {
      if (
        verification_docs?.aadhaar?.front_url &&
        verification_docs?.secondary_id?.front_url
      ) {
        return "full";
  ...
  const handleQuickVerification = async (e: React.FormEvent) => {
    ...
    if (!user || !quickAadhaarFront) {
      toast({
        title: "Missing File",
        description: "Please upload your Aadhaar front image.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Quick verification completed successfully!",
      });
      fetchUserProfile();
    } catch (error: unknown) {
      const errorMessage =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      toast({
        title: "Missing Files",
        description: "Please upload all four document images.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const aadhaarFrontUrl = await uploadFile(
        aadhaarFront,
        `${basePath}_aadhaar_front`,
      );
      const aadhaarBackUrl = await uploadFile(
        aadhaarBack,
        `${basePath}_aadhaar_back`,
      );
      const secondaryIdFrontUrl = await uploadFile(
        secondaryIdFront,
        `${basePath}_secondary_front`,
      );
      const secondaryIdBackUrl = await uploadFile(
        secondaryIdBack,
        `${basePath}_secondary_back`,
      );
      ...
      const docsPayload: VerificationDocs = {
        ...
        secondary_id: {
          ...
          front_url: secondaryIdFrontUrl,
          back_url: secondaryIdBackUrl,
        },
        submitted_at: new Date().toISOString(),
      };
      ...
      if (updateError) {
        ...
        throw new Error(
          `Error uploading verification docs: ${updateError.message}`,
        );
      }

      toast({
        title: "Success",
        description: "Documents uploaded and are now pending review.",
      });
  ...
  const renderTierCard = (tier: VerificationTier) => {
    ...
    const isCompleted =
      verificationTiers.findIndex((t) => t.id === currentTierId) >=
      verificationTiers.findIndex((t) => t.id === tier.id);
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <Badge variant={isCurrentTier ? "default" : "outline"}>
                {tier.docsRequired === 0
                  ? "No documents"
                  : `${tier.docsRequired} document${tier.docsRequired > 1 ? "s" : ""}`}
              </Badge>
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {verificationTiers.find((t) => t.id === currentTierId)?.name} -{" "}
                {
                  verificationTiers.find((t) => t.id === currentTierId)
                    ?.description
                }
              </p>
            </div>
            <Badge
              variant={getTierBadgeVariant(currentTierId)}
              className="ml-auto"
            >
        ...
        {currentTierId !== "full" && (
          <div className="border rounded-lg p-4">
            ...
            <Tabs
              value={currentTier}
              onValueChange={(value) =>
                setCurrentTier(value as "auto" | "quick" | "full")
              }
            >
              ...
              <TabsContent value="quick" className="mt-4">
                <form onSubmit={handleQuickVerification} className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    Upload just your Aadhaar front image for quick verification.
                    This gives you access to most treks.
                  </p>
                  <div>
                    <Label htmlFor="quick-aadhaar-front">
                      Aadhaar Front Image
                    </Label>
                    <Input
                      ...
                      accept="image/*"
                      onChange={(e) =>
                        setQuickAadhaarFront(e.target.files?.[0] || null)
                      }
                      required
                    />
                  </div>
                  <Button
                    type="submit"
                    disabled={loading || !quickAadhaarFront}
                  >
              ...
              <TabsContent value="full" className="mt-4">
                <form onSubmit={handleSubmit} className="space-y-6">
                  <p className="text-sm text-muted-foreground">
                    Upload all four document images for complete verification.
                    This gives you access to all treks including government
                    permit events.
                  </p>
                    ...
                    <div className="space-y-4 mt-2">
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={setSecondaryIdType}
                        >
                          <SelectTrigger>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <Label htmlFor="secondary-id-front">
                            Front Image
                          </Label>
                          <Input
                            id="secondary-id-front"
                            type="file"
                            accept="image/*"
                            onChange={(e) =>
                              setSecondaryIdFront(e.target.files?.[0] || null)
                            }
                            required
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            id="secondary-id-back"
                            type="file"
                            accept="image/*"
                            onChange={(e) =>
                              setSecondaryIdBack(e.target.files?.[0] || null)
                            }
                            required
                          />
                        </div>
                  ...
                  <Button type="submit" disabled={loading}>
                    {loading ? "Uploading..." : "Complete Full Verification"}
                  </Button>
                </form>
              </TabsContent>
            </Tabs>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700 mt-1">
                Some treks require government ID verification for ticket booking
                and permits. Your current verification level determines which
                treks you can join.
              </p>
            </div>
          </div>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Choose your verification level based on the treks you want to join.
          Auto-verified users can access basic treks, while full verification
          unlocks government permit events and priority booking.
        </CardDescription>
      </CardHeader>
