...
export function useGA4Analytics() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!isGA4Initialized) {
        if (import.meta.env.DEV) {
          console.log('[GA4] Waiting for initialization before tracking pageview');
        }
        return;
      }
  ...
  }, [location.pathname, location.search, user?.id, userProfile, GA4_MEASUREMENT_ID, ANALYTICS_ENABLED]);
  ...
  }, [userProfile?.role, location.pathname, GA4_MEASUREMENT_ID, ANALYTICS_ENABLED]);
   ...
   */
  const trackTrekRegistration = useCallback((trekId: string, trekName: string, cost: number) => {
   ...
   */
  const trackGalleryView = useCallback((imageId: string, trekName?: string) => {
   ...
   */
  const trackForumInteraction = useCallback((action: string, threadId?: string) => {
