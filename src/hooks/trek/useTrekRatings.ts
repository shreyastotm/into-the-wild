  ...
  const fetchTrekRatings = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const summary = data.reduce((acc, rating) => {
        ...
        if (summary.totalRatings > 0) {
          summary.difficulty = Math.round((summary.difficulty / summary.totalRatings) * 10) / 10;
          summary.enjoyment = Math.round((summary.enjoyment / summary.totalRatings) * 10) / 10;
          summary.scenic = Math.round((summary.scenic / summary.totalRatings) * 10) / 10;
        }
  ...
  const rateTrek = async (
    ...
    try {
      ...
      toast({
        ...
        description: hasRated ? 'Your rating has been updated' : 'Thank you for rating this trek',
        variant: 'default'
      });
