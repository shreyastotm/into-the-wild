...
export function useTransportCoordination(trekId: string | undefined) {
  ...
  const [userAssignedDriver, setUserAssignedDriver] = useState<Driver | null>(
    null,
  );
  const [userPickupLocation, setUserPickupLocation] =
  ...
  const fetchTransportData = useCallback(async () => {
    ...
    try {
      ...
      if (rawDriversData && rawDriversData.length > 0) {
        ...
        const validDrivers = rawDriversData.filter(
          (d: Record<string, unknown>) =>
            d.user_id && d.user_id.toString().trim() !== "",
        );
        ...
        if (validDrivers.length === 0) {
          ...
          const driverUserIds = [
            ...new Set(validDrivers.map((d) => d.user_id).filter(Boolean)),
          ];
          let driverUserDetails: {
            user_id: string;
            name: string | null;
            avatar_url: string | null;
          }[] = [];
          if (driverUserIds.length > 0) {
            const { data: driverUsersData, error: driverUsersError } =
              await supabase
            ...
            if (driverUsersError)
              console.error(
                "Error fetching driver user details:",
                driverUsersError,
              );
            else driverUserDetails = driverUsersData || [];
          }
          driversWithNames = validDrivers.map(
            (driver: Record<string, unknown>) => {
              const userDetail = driverUserDetails.find(
                (u) => u.user_id === driver.user_id,
              );
              ...
              if (driver.vehicle_info) {
                try {
                  ...
                  if (typeof driver.vehicle_info === "object") {
                    vehicleInfo = driver.vehicle_info as Record<
                      string,
                      unknown
                    >;
                  ...
                  console.warn(
                    "Failed to parse vehicle_info, using individual columns:",
                    error,
                  );
                }
              }
        ...
        const { data: rawParticipantsData, error: rawParticipantsError } =
          await supabase
        ...
        if (rawParticipantsData && rawParticipantsData.length > 0) {
          ...
          const validParticipants = rawParticipantsData.filter(
            (p: Record<string, unknown>) =>
              p.user_id && p.user_id.toString().trim() !== "",
          );
          const participantUserIds = [
            ...new Set(validParticipants.map((p) => p.user_id).filter(Boolean)),
          ];
          let participantUserDetails: {
            user_id: string;
            name: string | null;
            avatar_url: string | null;
          }[] = [];
          if (participantUserIds.length > 0) {
            // Fetch user details
            const { data: participantUsersData, error: participantUsersError } =
              await supabase
                .from("users")
          ...
          participantsWithDetails = validParticipants.map(
            (p: Record<string, unknown>) => {
              const userDetail = participantUserDetails.find(
        ...
        const { data: rawAssignmentsData, error: assignmentsError } =
          await supabase
          ...
          if (rawAssignmentsData) {
            rawAssignmentsData.forEach((assignment) => {
              if (assignment.driver_id === driver.user_id) {
                const participant = participantsWithDetails.find(
                  (p) => p.user_id === assignment.participant_id,
                );
                if (participant) {
                  ...
                  driver.pickup_status[participant.user_id] =
                    ((assignment as Record<string, unknown>)
                      .status as PickupStatus) || "pending";
                }
              }
            });
          }
        ...
        if (user) {
          const isDriver = driversWithNames.some(
            (driver) => driver.user_id === user.id,
          );
          const isParticipant = participantsWithDetails.some(
            (p) => p.user_id === user.id,
          );
          const assignedDriver =
            driversWithNames.find((driver) =>
              driver.assigned_participants.some((p) => p.user_id === user.id),
            ) || null;
          const userLocation =
            participantsWithDetails.find((p) => p.user_id === user.id)
      ...
      toast({
        ...
        description:
          "There was an error loading the transport coordination data.",
        variant: "destructive",
      });
    ...
    try {
      ...
      if (registrations && registrations.length > 0) {
        ...
        const { data: users, error: userError } = await supabase
          .from("users")
          .select(
            "user_id, name, has_car, car_seating_capacity, vehicle_number, transport_volunteer_opt_in",
          )
          .in("user_id", userIds)
          .eq("has_car", true)
        ...
        const drivers = (users || [])
          ...
          .map((user) => ({
            ...
            vehicle_type:
              user.car_seating_capacity <= 5
                ? "Car (5 Seater)"
                : user.car_seating_capacity <= 8
                  ? "SUV/MUV (6-8 seater)"
  ...
  const processLocationsData = (
    locationsData: Array<Record<string, unknown>>,
  ): PickupLocation[] => {
  ...
  const assignDriverToParticipant = async (
    driverId: string,
    ...
    try {
      ...
      const { error } = await supabase.from("trek_driver_assignments").upsert({
  ...
  const setParticipantPickupLocation = async (
    participantId: string,
    locationId: string,
  ) => {
    if (!trekId) return false;
  ...
  const createPickupLocation = async (location: {
    name: string;
    address: string;
    latitude?: number;
    longitude?: number;
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(
          `Failed to create pickup location: ${error.message}. Please check if the database schema is properly updated.`,
        );
      }
      await fetchTransportData();
  ...
  const upsertDriver = async (driver: {
    ...
    try {
      ...
      if (error) {
        console.error("Database error details:", error);
        throw new Error(
          `Failed to save driver: ${error.message}. Please check if the database schema is properly updated.`,
        );
      }
      await fetchTransportData();
